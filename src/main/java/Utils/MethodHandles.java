...
public class MethodHandles {
    ...
    protected JavascriptExecutor js;   // ← declare it
   ...
   // private List<WebElement> webElements(By locator) {return driver.findElements(locator);}
...
*/
    ...
    protected String getAttributeValue(By locator, int time) {
        String values = null;
        for (int i = 0; i < 5; i++) {
            try {
                explicitWait(locator, time);
                values = webElement(locator).getAttribute(String.valueOf(locator));
                break;
            } catch (StaleElementReferenceException e) {
    ...
    /*
    protected void moveToElements(By locator, int time) {
        actions = new Actions(driver);
        for (int i = 0; i < 5; i++) {
            try {
                explicitWait(locator, time);
                setSteps();
                //    addBorderToElement(driver, webElement(locator));
                actions.moveToElement(webElements(locator)).build().perform();
                break;
            } catch (StaleElementReferenceException e) {
